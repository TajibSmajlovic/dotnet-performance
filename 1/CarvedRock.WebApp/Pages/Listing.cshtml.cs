...
public partial class ListingModel : PageModel
{
    ...
    public async Task OnGetAsync()
    {
        ...
        if (response.IsSuccessStatusCode)
        {
            ...
            //Products = JsonSerializer.Deserialize<List<ProductModel>>(jsonContent, new JsonSerializerOptions(defaults: JsonSerializerDefaults.Web));
            //Products = await response.Content.ReadFromJsonAsync<List<ProductModel>>();

            Products = JsonSerializer.Deserialize(jsonContent, SourceGenerationContext.Default.ListProductModel); // The fastest!!
